## v4.38.0 (2026-09-13) — этап 139: «музыка отсутствует» — два корневых дефекта аудио

### Сводка
Ответ на вопрос владельца «а МУЗЫКА ПОЧЕМУ ОТСУТСТВУЕТ?». Диагноз
доказательный (grep + git log -S + расчёт дБ), не гипотетический:

1) Пэд-слой молчал С РОЖДЕНИЯ движка (v3.1.0): padGain создавался
   setValueAtTime(0) и НИКОГДА не поднимался — ни одного ramp по padGain за
   всю историю файла (git log -S «padGain.gain.linearRampToValueAtTime» —
   0 совпадений). Аккорды — основная масса музыки — выдавали
   гарантированную тишину.
2) Даже «живые» слои лежали на −30…−48 дБ: бас = bassGain 0.012–0.025 ×
   userVolume 0.7 (−42…−35 дБ), мелодия = melodyGain 0.004–0.008 × 0.7
   (−48…−41 дБ). SFX-пресеты при этом 0.15–0.22 × 0.8 (−17…−13 дБ) — в
   20–30 раз громче по амплитуде. Игрок слышал только SFX и файловые
   .ogg-эмбиенты.
3) Процедурный эмбиент — та же яма: слои конфигов 0.012–0.04 × 0.6 ≈
   −38…−32 дБ — фактически неслышим.

### fix(music): пэд-шина взводится, makeup ×6, лимитер
- MusicEngine.startMusicForScene: padGain ramp 0→1 за 2 с (атаки голосов и
  мастер-фейд той же длительности дают плавный вход без щелчка).
- MUSIC_BUS_MAKEUP_GAIN = 6 (+15.6 дБ) в обоих местах расчёта
  effectiveGain (fade-in бэда и applyVolume): volodka_room
  0.04 × 6 × 0.5 = 0.12 линейно вместо 0.02. Все конфиги сцен и
  меню-музыка получают усиление единообразно.
- Лимитер DynamicsCompressorNode (threshold −6 дБ, knee 6, ratio 12,
  attack 3 мс, release 250 мс) между master и destination — сумма
  пэд+бас+мелодия не клиппует; fallback на прямое подключение в
  экзотических браузерах без компрессора.
- Лимитер рвётся в dispose() и на пути «контекст закрыт» (иначе утечка
  узлов при пересоздании контекста).
- Прод-маркер диагностики: console.info «[audio:music] bed …» при старте
  каждого бэда — владелец может проверить факт старта музыки и итоговый
  гейн прямо в консоли прод-деплоя.

### fix(ambient): makeup ×4 на шину эмбиента + stale destination
- AMBIENT_BUS_MAKEUP_GAIN = 4 (+12 дБ): процедурные дроны выводятся на
  −26…−20 дБ — под музыкой (−22…−18) и SFX (−17…−13).
- Попутный фикс: destination-шина настраивалась ОДИН раз в initContext и
  не реагировала на setVolume (stale-гейн со старым baseVolume) — теперь
  обновляется при каждой смене громкости.

### Тесты
- +4 регресс-теста MusicEngine (инструментированный мок AudioContext с
  createDynamicsCompressor): ramp padGain ≥ 0.99 существует при старте
  бэда; мастер-фейд = 0.12 (makeup ×6 учтён); лимитер в цепи
  master→compressor→destination с threshold −6 / ratio 12; dispose рвёт
  лимитер немедленно.
- +1 регресс AmbientEngine: destination = 0.7 × 4 = 2.8 при init;
  setVolume(0.5) → setValueAtTime(2.0).
- Верификация: tsc 0; ESLint 0 errors; vitest ПОЛНЫЙ 2740/2740 (438
  файлов, +5); build OK; budgets OK (boot/game-start в hard-max —
  документировано ранее). poems.ts — diff 0.
...
### Сводка
По жалобе «не заметно разницы» проведена форензика прода (volodka.vercel.app):
prod раздаёт сборку, предшествующую c886b871 (2026-09-06) — **132 коммита
позади main**, включая ВСЕ три волны фиксов (KTX2 внешних карт v4.34,
Suspense-петля текстур v4.35, масштабы/тайлинг v4.36). Доказательства:
в прод-чанках `engine-combat-CEzG7ZFS`/`game-canvas-DZGldZIh` — старые фасады
`hero:2.38,mid:1.96,side:1.78` (в v4.36.0 заменены на `hero:1.18`), маркер
standalone-KTX2-лоадера `ktx2:texture-loader-init` (v4.34+) отсутствует,
`/sw.js` — shell-v2/media-v2 (текущий repo — v3/v4 с v4.33). При этом GitHub
Status API показывает `Vercel => success «Deployment has completed»` на
e70c336a: деплои УСПЕШНЫ, но домен production не переключается (предпросмотр
git-main SSO-защищён → деплой ветки main идёт в preview, а не production;
Production Branch в Vercel не настроен на main или домен закреплён за старым
деплоем). Правка на стороне Vercel (2 минуты, без сборки): Dashboard →
Volodka → Deployments → свежий Ready-деплой → «⋯» → **Promote to Production**;
и/или Settings → Git → Production Branch → `main`. Проверка результата:
в левом нижнем углу меню чип версии должен показать `v4.36.1 · <sha>`.

### fix(ci): триггер workflows — branches [main] вместо повреждённого литерала
- `.github/workflows/ci.yml` содержал `branches: ain]` (съедено «[m») в push
  И pull_request — CI не запускался вовсе. Восстановлены триггеры на main.
  Примечание: тот же механизм «проглатывания» `[m` при передаче текста —
  вероятный источник исходного повреждения файла.

### feat(build): билд-метка «версия · sha» в чипе версии меню
- vite.config: `define __BUILD_SHA__` ← `git rev-parse --short HEAD`
  (fallback 'dev' вне git/сборки).
- appVersion.ts: `APP_BUILD_SHA` (typeof-гард для vitest) + `APP_BUILD`
  («4.36.1 · a1b2c3d»); menuConstants реэкспортирует; чип меню теперь
  `v{APP_BUILD}` — какой коммит играет, видно без DevTools.
- Мотивация: prod застрял на 132 коммита, и это обнаружилось только по
  субъективному «не заметно разницы». Версия в чипе (v4.1x) давала бы
  ответ сразу.

## v4.36.0 (2026-09-13) — волна доказательных фиксов №3: масштабы по измеренным нативам GLB + тайлинг

### Сводка
Закрыты все «измеренные-но-не-тронутые» пункты follow-up этапа 135 (фасады/
лампы/скамьи/тайлинг). Метод инвариантен этапу 135: натив каждого GLB измерен
рекурсивным обходом нод (accessor min/max × node-TRS, включая вложенные
children) — правки применены только там, где числа подтвердили дефект.
Верификация: tsc (0), ESLint на всех изменённых файлах (0), vitest ПОЛНЫЙ
2731/2731 (438 файлов, +1 новый audit-тест), validate:content (0), vite build
+ budgets (OK). poems.ts не тронут.

### fix(scale): STREET_FACADE_SCALE 1.78–2.38 → 0.88–1.18 + groundAnchor
- Натив `modular_urban_apartments_facade.glb` ИЗМЕРЕН: 51.53 × 17.0 × 6.66 м,
  minY −2.0 (4 нода-модуля: door_angled_small, door_window_small_09,
  crown_standard_angled_large, wall_door_centered_large). Прежние множители
  рассчитывались из посылки «~3 м shell» → инстансы 30–41 м высоты и до
  123 м длины при процедурном силуэте тех же слотов 15–25 м.
- Новые: hero ×1.18 ≈ 20.1 м (5–6 этажей), mid ×1.03 ≈ 17.5 м,
  side ×0.88 ≈ 15.0 м; hero-длина 60.8 м (граница квартала ~70 м — тест).
- ВАЖНО: фасады в street_night (GltfProp) и city_square (AuthoredPlazaProp)
  размещались БЕЗ groundAnchor → при minY −2.0 инстанс тонул на
  −2.36…−4.76 м: наземный этаж с дверями был под землёй. Теперь
  groundAnchor у всех 13 фасадных инстансов.
- audit-таблица: строка street_facade → fixed, targetM 20; добавлены строки
  street_lamp_alt и bench_scale; новый тест масштабов от измеренного натива
  (URBAN_FACADE_NATIVE_HEIGHT_M = 17.0).

### fix(scale): street_lamp_02 ×2.6 — фонарь больше не ниже игрока
- Натив street_lamp_02: 0.39 × 1.68 × 0.81 м, minY −0.395. Прежний manualScale
  ×1.0–1.05 давал 1.28–1.35 м НАД ЗЕМЛЁЙ — фонарь ниже игрока (1.75 м).
  Registry-путь (targetSizeM 3.4, height-fit → ×2.02) был корректен — дефект
  только в manualScale-путях: PolyHavenStreetDressing ×1.0 → ×2.6 (≈4.37 м),
  city_square лампы → условный scale (lamp_01 ×1.05 = 4.06 м, alt ×2.6).

### fix(scale): скамьи — height-fit 0.9 м вместо ростом-с-человека
- Натив painted_wooden_bench ИЗМЕРЕН: 1.16 × 0.89 × 0.5 м (рекурсивно —
  подтверждено отсутствие вложенных нод). Было: InstancedProp ×1.35/1.15/1.1
  → 0.98–1.20 м ростом; registry width-fit [2.05, 0.88, 0.78] → scale 1.77 →
  скамья 1.57 м РОСТОМ. Стало: InstancedProp ×1.0/0.95/0.9; registry
  [1.75, 0.9, 0.7] height-fit → scale ≈ 1.01, длина 1.17 м, высота 0.90 м.

### fix(textures): тайлинг-прыжок PBR↔процедурный фолбэк устранён
- Процедурный concrete: repeat 6 → 5 (= PBR DEFAULT_REPEAT
  concrete_floor_painted) — зимний грунт WetStreetGround больше не прыгает
  на 20% при приходе KTX2-набора.
- Процедурный тротуар (StreetSidewalkProcedural): было (repeat×0.35,
  ×2.2) = (1.75, 11) против PBR 5.5 uniform — прыжок ~3× при загрузке.
  Теперь UV-плотность из нового единого источника
  `getPolyHavenRepeat('concrete_floor_painted', repeatScale)` (polyhavenAssets)
  — фолбэк и PBR-ветка используют одну формулу (5×1.1 = 5.5; зима 5×0.9 = 4.5).
- Уникальные текстурные клоны фолбэка пересоздаются по [maps, pbrRepeat].
  UniqueStreetFacades/HeroStreetFacades не читают repeat concrete — не задеты.

### Измерено, но СОЗНАТЕЛЬНО не тронуто (нужна визуальная QA)
- Офисный стол 0.556 м vs PLAYER_METRIC.deskHeightM 0.76 — авторская
  раскладка комнаты (follow-up v4.35.0).
- Анизотропная растяжка PBR-тротуара: тайл 1.09 × 7.27 м на плите 6×40 м
  (5.5 uniform) — выравнивание фолбэка убрало прыжок, но сама плотность
  вдоль X требует глаз: возможен переход на (5.5×6.67, 5.5).

## v4.35.0 (2026-09-13) — волна доказательных фиксов по фидбеку: текстуры/физика/камера/масштабы/звук

### Сводка
Очередная волна по жалобе «текстуры/масштабы/позиционирование/камера/музыка/
модели/физика работают некорректно». Метод: два параллельных статических
исследования с замерами GLB-габаритов (accessor min/max × node-TRS) +
побайтовая проверка KTX2-заголовков. Подтверждено и исправлено 6 дефектов,
каждый — с файлом/строкой и числами. Верификация: tsc (0), ESLint на всех
изменённых файлах (0), vitest ПОЛНЫЙ 2730/2730 (438 файлов), vite build +
budgets (OK; boot/game-start в hard-max — документированное состояние).
poems.ts не тронут.

### fix(textures): вечная Suspense-петля — материалы PBR не появлялись вовсе
- `usePolyHavenPbr` вызывал `use()` с промисом, созданным В ТЕЛЕ рендера
  (Promise.all-обёртка над кэшированными URL-промисами). Каждая повторная
  попытка React после разрешения получала СВЕЖИЙ pending-промис → петля
  «suspend → resolve → render → новый pending» никогда не сходится: материалы
  PolyHaven не появлялись вовсе (регрессия этапа 133, f2b884e8 — совпадает с
  началом жалоб). Фикс: промис в `useMemo` (стабильная ссылка на
  [urls, fallbackUrls, gl]); дедупликация URL-промисов в ktx2Textures не
  изменена. Сами KTX2-файлы проверены побайтово — валидные (магия KTX2,
  vkFormat 0 = транскодируемый Basis, 1024², 11 mips, Zstd).

### fix(physics): ground-probe рейкаст был мёртв (interaction groups)
- `ENVIRONMENT_COLLISION_GROUP = 0b…0001` задавал membership = 0x0000 (в
  семантике Rapier membership — СТАРШИЕ 16 бит) → условие взаимодействия
  с дефолтными коллайдерами (0xFFFF_FFFF) всегда ложно → `probeGroundY`
  всегда возвращал `fallbackFloorY`; весь ground-probe/rescue/micro-hover
  работал по плоской модели пола (на многоэтажных сценах «rescue» телепортировал
  бы между этажами). Фикс: `0x0001_0001` (membership+filter группы 1).

### fix(camera): уже идущий орбит гейтится shouldBlockOrbit()
- Гейт стоял только на ВХОДЕ жеста (mousedown/touchstart/Shift+R). Drag или
  pointer-lock-лок, начатые до диалога/кат-сцены/боя, продолжали мутировать
  yaw/pitch до mouseup, и мутации протекали в пост-катсценную позу (тот же
  класс бага, что уже чинился для Shift+R). Гейт добавлен на каждую мутацию:
  pointer-lock-freelook, LMB-орбит, общий drag, одиночный палец touch.

### fix(scene): city_square — паритет с фикс-ом v4.14.0 street_night
- `AuthoredPlazaProp` не имел ground-anchor (в отличие от GltfProp street_night):
  пожарные лестницы scale 1.25/1.18 → 12-метровые с уходом под плиту на ~4.6 м
  (натив 9.76 м, minY −3.65) — теперь 0.55/0.52 (документированный коридор
  0.5–0.62) + якорь; мусорные баки 1.15 → 0.67 (те самые «2.2-метровые
  монстры» — GLB содержит ПАРУ контейнеров) + якорь; old_tyre/power_box/
  utility_box/wooden_crate/wet_floor_sign/street_lamp_02 (minY −0.395 —
  фонари были утоплены на 0.41 м) — якоря по измеренным minY.
- Офис (office_day): терминал парил ~0.24 м над столом — ставлен на
  столешницу (натив 0.958 × scale 0.58 = 0.556 м). Высота самого стола
  (0.556 против PLAYER_METRIC.deskHeightM 0.76) — осознанно НЕ менялась
  вслепую: авторская раскладка комнаты; follow-up.

### fix(audio): VO подчинён мастер-мьюту и громкости
- Голосовые линии (и файловые, и Web Speech TTS) жёстко задавали громкость
  (0.9/0.95) мимо «Без звука» и ползунков. Теперь: мьют → 0, иначе — доля
  SFX-громкости; активный <audio> глушится живьём (подписка на
  audio-settings-changed); уже играющий utterance Web Speech API изменить
  нельзя (применяется с новой реплики — ограничение API, задокументировано).

### Измерено, но СОЗНАТЕЛЬНО не тронуто (нужны глаза/дизайн-решение)
- `STREET_FACADE_SCALE` (1.78–2.38) рассчитывался из неверной посылки
  «~3 м shell»: реальный натив фасада — 51.5×17.0 м → инстансы 30–41 м высоты
  и до 123 м длины, при процедурном силуэте тех же слотов 15–25 м и audit-цели
  8 м. Масштаб вслепую не резался — нужен выбор силуэта + визуальная QA.
- Тайлинг-«прыжок» PBR↔процедурный фолбэк (concrete 5 vs 6; тротуар 5.5
  uniform против 1.75×11). Какая сторона каноническая — неясно без глаз.
- Офисный стол 0.556 м; bench targetSizeM 2.05 м (даёт скамью 1.56 м ростом);
  street_lamp_02 ниже игрока (1.28 м) — дизайн-вопросы, в очередь.
