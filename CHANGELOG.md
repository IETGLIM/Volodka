...
- **Arcade Demo (vertical slice ~1–2 ч):** пресеты `src/config/gameModePresets.ts`, `useSessionPresetStore`, пункт меню «Демо · Глава 1», изолированный сейв `volodka_save_demo_v1`, сжатые узлы `verticalSliceStoryNodes.ts` с аркадными битами (`vs_arcade_drill_qte`, `vs_arcade_chat_spam`, `vs_arcade_incident`), `ArcadeMinigamePanel`, `DemoResultsStrip`, `arcadeScoreStore`, тест `src/data/arcadeSlicePath.test.ts`.
...
- **Поддерживаемость коллайдеров интерьеров (без изменения поведения):** в `src/ui/game/PhysicsSceneColliders.tsx` «магические» смещения дверного проёма комнаты Володьки вынесены в именованные константы с контрактным комментарием по осям/смещениям, а для тонких столешниц (`0.08`) и политики `wall clamp` vs `obstacle authored size` добавлены явные пояснения; в `src/lib/explorationInteriorReference.ts` уточнена область применения `INTERIOR_PHYSICS_COLLIDER_WALL_MIN_EXTENT_M`.
- **Калибровка physics constants и разделение camera/physics:** в `src/engine/physics/constants.ts` применён balanced-пресет (`WALK_SPEED` 2.6, `RUN_SPEED` 5.2, `PLAYER_RADIUS` 0.35, `GRAVITY` -18) с сохранением текущих `JUMP_FORCE`/`MAX_UPWARD_SPEED`; удалены неиспользуемые `GROUND_FRICTION`/`AIR_FRICTION`, camera-константы вынесены в `src/engine/camera/constants.ts`, а в `src/engine/physics/CharacterController.ts` зафиксирован контракт масштабирования `locomotionScale` vs `speedScale`.
- **PhysicsPlayer / анимации обхода:** в `src/ui/game/PhysicsPlayer.tsx` стабилизирован `onPositionChange` через ref (без лишних замыканий в `useFrame`), добавлен `arePropsEqual` для `memo` (меньше лишних ререндеров при новых ссылках пропсов), а в `src/game/simulation/explorationGlbAnimation.ts` расширен планировщик клипов направлением движения (`forward/backward/strafe`) с безопасным fallback на прежние walk/run/idle клипы.
- **CharacterController / прыжок и устойчивость шага:** в `src/engine/physics/CharacterController.ts` добавлены coyote time, jump buffer и variable jump height (jump-cut), опциональные `movementModifiers` (speed/accel/friction/gravity), ограничение максимального смещения за тик (`capsuleRadius`-cap против tunneling на просадках fps), масштабирование параметров KCC (`autostep`, `snapToGround`) по `roomScale` и опциональная подстройка капсулы под `visualModelScale`; `src/ui/game/PhysicsPlayer.tsx` прокидывает новые таймеры и масштаб контроллера.
- **KCC API / DX-контракт:** в `src/engine/physics/KCC.ts` добавлены `@see`-комментарии на публичные реэкспорты и экспорт связанных типов (`MutableScalarRef`, `MutableBoolRef`, `MovementModifiers`, `CharacterControllerWorld`); в `eslint.config.mjs` включён `no-restricted-imports` против прямых импортов `@/engine/physics/CharacterController*`; в `package.json` задано `sideEffects: false` для более надёжного tree-shaking.
...
- **AAA+ стартовый кадр / прод-UI / глубина:** в `src/ui/game/RPGGameCanvas.tsx` увеличен визуальный масштаб героя в `intro_cutscene` (без ломки геймплейного масштаба), повышена точность depth (`camera.near/far`) для снижения z-fighting; в `src/ui/game/HUD.tsx` и `src/ui/game/PlayerOrbitHeader.tsx` ужаты и разведены верхние блоки, чтобы не было наслоений health/karma/exp/кнопок; в `src/ui/game/IntroScreen.tsx` отключён туман в text-focus фазах и выровнен тон атмосферы с загрузочным экраном; debug-панель скрыта из production по умолчанию через `src/lib/narrativeDebug.ts` + гейтинг в `src/ui/game/GameOrchestrator.tsx` (опциональный override `NEXT_PUBLIC_ENABLE_PROD_DEBUG_PANEL=1`).
...
- **Обход iteration 3 — game feel:** асимметричный разгон/торможение по горизонтали (`CharacterController.ts`: accel/decel); чуть мягче yaw визуала (`explorationMovementPresentation.ts`); короче crossfade GLB (`PhysicsPlayer.tsx`). Камера TPS: чуть «тяжелее» damp (`FollowCamera.tsx`), микро-тряска при успешном E через `ui:interaction_feedback` и `explorationCameraShake.ts`. Событие шины `ui:interaction_feedback` + `interactionFeedback.ts`; вызовы из `RPGGameCanvas.tsx` при подборе, реестре, триггере, объекте, NPC и «пустом» E.

- **Обход iteration 3 — feedback UX:** `InteractionFeedbackListener.tsx` + звук `AudioEngine.playSfx('ui_success'|'ui_fail')` (`lib/uiSfxMap.ts` — несколько путей на id, резерв — бип с разбросом частоты ±20 Hz в `playSfxBeep`); **throttle 60 ms** на `ui_success` / `ui_fail` от оверлапа. Короткий буллет `ui:effect_notif` → `showEffectNotif` в `GameOrchestrator.tsx` (прокид `priority`, `displayStyle` strong/subtle/normal, `high` — длительность ≥1200 ms). **Дедуп** success/fail по `actionId` + TTL GC + `clear` при unmount; `emitInteractionFeedback(…, actionId)` из `RPGGameCanvas` / `interactionFeedback.ts`. **Тряска:** `ensureInit` (без подписки в SSR), rate limit 80 ms, дельта по `loot` / `npc` / остальное. **Pre-hint:** `ui:interaction_hint` + `useInteractionAnticipation` в `RPGGameCanvas`. **Шина:** `actionId?` в `ui:interaction_feedback`, `UiPriority` на `ui:effect_notif`.

- **Обход iteration 2 — освещение и пост:** `getExplorationSceneGlProps` (`Scene.tsx`) — `ACESFilmicToneMapping`, `SRGBColorSpace`, экспозиция (ниже при `visualLite`). `explorationIblProfiles.ts` + `ExplorationEnvironmentIbl.tsx` (`drei` `Environment preset`); ребаланс ambient/клав при IBL и тюн `district` / `mvd` в `RPGGameCanvas.tsx`. Тени Canvas — `PCFSoftShadowMap`; bias в `explorationShadowConstants.ts`. `ExplorationPostFX` — лёгкий `DepthOfField` при `enableSubtleDepthOfField` (десктоп по ширине). PBR персонажей: `applyGltfExplorationPbrNormalization` в `gltfCharacterMaterialPolicy.ts`. **Коридор Володьки:** три плафона — `InstancedMesh` в `VolodkaCorridorVisual.tsx`. Тест: `explorationIblProfiles.test.ts`.
...
- **Git / LFS**: в **`.gitattributes`** нет `filter=lfs` для GLB; комментарий фиксирует политику «только обычные blobs», без повторного включения LFS.
