...
- **Обход / WASD и камера**: горизонталь KCC в **`integrateKinematicLocomotionDelta`** для **`PhysicsPlayer`** — от **текущего yaw модели** (`horizontalWorldSpace: false`, перед шагом физики **`moveYawRef = rotationRef`**), а не от мировых осей; при **`spawnSyncKey`** сбрасываются **`rotationRef` / `moveYawRef`** на **`initialRotation`**. **`ExplorationNoclipPlayer`** — тот же принцип (шаг по yaw + поворот по вектору ходьбы). Прыжок: **`JUMP_FORCE`** ниже, **`MAX_UPWARD_SPEED`**, выше **`PLAYER_HEIGHT`** капсулы — меньше визуальный клип головы в потолок. В **`RPGGameCanvas`** для **`volodka_room`**, **`volodka_corridor`**, **`home_evening`** — ближе **`distance` / `maxDistance`** и чуть ниже **`height`**.
