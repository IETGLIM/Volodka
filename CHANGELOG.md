...
- **Обход iteration 3 — feedback UX:** `InteractionFeedbackListener.tsx` + звук `AudioEngine.playSfx('ui_success'|'ui_fail')` (`lib/uiSfxMap.ts`, резерв — бип); короткий буллет `ui:effect_notif` → `showEffectNotif` в `GameOrchestrator.tsx`. Тряска камеры централизована в `explorationCameraShake.ts` (подписка на шину + экспоненциальный decay).
