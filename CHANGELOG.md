...
- **Обход iteration 3 — feedback UX:** `InteractionFeedbackListener.tsx` + звук `AudioEngine.playSfx('ui_success'|'ui_fail')` (`lib/uiSfxMap.ts` — несколько путей на id, резерв — бип с разбросом частоты ±20 Hz в `playSfxBeep`); короткий буллет `ui:effect_notif` → `showEffectNotif` в `GameOrchestrator.tsx` (прокид `priority`, `high` — длительность ≥1200 ms в `useEffectNotifications.ts`). **Дедуп success** по `actionId` (`Map`, 250 ms) + `emitInteractionFeedback(…, actionId)` из `RPGGameCanvas` / `interactionFeedback.ts`; **fail** — 400 ms global; success-toast `priority: 'low'`. **Тряска:** rate limit 80 ms, дельта по контексту (`loot` / `npc` / остальное) в `explorationCameraShake.ts`. **Шина:** `actionId?` в `ui:interaction_feedback`, `UiPriority` на `ui:effect_notif`.
