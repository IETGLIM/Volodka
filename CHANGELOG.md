...
- **Обход iteration 3 — feedback UX:** `InteractionFeedbackListener.tsx` + звук `AudioEngine.playSfx('ui_success'|'ui_fail')` (`lib/uiSfxMap.ts` — несколько путей на id, резерв — бип); короткий буллет `ui:effect_notif` → `showEffectNotif` в `GameOrchestrator.tsx`. **Дедуп success:** debounce 250 ms + «OK» 600 ms (вместо дубля с `ui:exploration_message`); fail: «Нет доступных действий». **Тряска:** rate limit 80 ms в `explorationCameraShake.ts` (подписка на шину + экспоненциальный decay). **Шина:** `UiPriority` / `UiEventMeta` на `ui:effect_notif` в `engine/events/EventBus.ts`.
