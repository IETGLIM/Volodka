...
- **`StreamingChunkId`** — строка в пространстве имён сцены, например `` `${SceneId}::courtyard` ``. Уникальна в пределах игры. Канонический тип: `src/data/streamingChunkId.ts` (реэкспорт из `src/config/scenes.ts` для профилей).
