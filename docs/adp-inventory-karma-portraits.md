...
| UI | Только `MoralCompassHUD` (шкала, стрелка, вспышка при заметном изменении). Отдельная «кибер-полоса» кармы в верхнем HUD **не** используется (избегать дублирования). |
