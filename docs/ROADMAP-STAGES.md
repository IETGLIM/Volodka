...
2. Реестр закрыт (132/132, v4.32.0). Фаза 10: этапы 133 (KTX2 внешних карт),
   134 (волна v4.33.0), 135 (волна v4.35.0) и 136 (волна v4.36.0) ЗАКРЫТЫ.
   Очередь: пер-сценная экспозиция ToneMapping, CoherentNoiseEffect,
   check-asset-manifest, вторая волна QTE-эмиттеров; ПЕРВЫМ ДЕЛОМ при
   наличии браузерной среды — QA KTX2-пути (follow-up этапа 133), снятие
   WebP-фолбэка из keep-set (−11 MB) и визуальная QA фиксов этапов 135–136
   (лестницы/фонари/баки на площади, силуэт и якоря фасадов, тайлинг).
