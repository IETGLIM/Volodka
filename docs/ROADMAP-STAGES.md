...
2. Реестр закрыт (132/132, v4.32.0). Фаза 10: этап 133 (KTX2 внешних карт)
   и этап 134 (волна стабилизации v4.33.0) ЗАКРЫТЫ. Очередь: пер-сценная
   экспозиция ToneMapping, CoherentNoiseEffect, check-asset-manifest,
   вторая волна QTE-эмиттеров; ПЕРВЫМ ДЕЛОМ при наличии браузерной среды —
   QA KTX2-пути (follow-up этапа 133) и снятие WebP-фолбэка из keep-set.
