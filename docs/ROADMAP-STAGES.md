...
- [ ] 85. Регресс-тест: линтер-правило на литеральные `\n` внутри JSX-текста
- [ ] 86. Мобильная ревизия нижнего стека (джойстик/кнопки/тулбар)
- [ ] 87. `FirstMinutesDirector` (`bottom-[14vh]`) → слот-сетка
- [ ] 88. `CriticalStatusWhisper` (`clamp(118px,16vh,176px)`) → слот-сетка
- [ ] 89. `AaaImmersiveGuide` (`bottom-[22vh]`) → слот-сетка
- [ ] 90. `EmergencyHelpButton`/`AmbientSoundMixer` — ревизия правого нижнего угла
- [~] 91. Консолидация UI-тиков кулдаунов (интервалы останавливаются при простое)
- [ ] 92. Скрипт-аудит слотов (детектор коллизий прямоугольников HUD-виджетов)
...
- [ ] 97. Крипы: LOS/страйк-репорты по дистанционному LOD (расширить npcRenderTier на AI-тик)
- [ ] 98. Persistent EffectComposer между сценами (убрать stall 250–2000мс на переходах)
- [ ] 99. Выборочная интерполяция динамических физтел (сейчас interpolate=false глобально)
- [ ] 100. Фасад-стор: точечные подписки HUD вместо useGameStore-фасада
- [ ] 101. `useWorldClock`: dirty-check NPC-стейтов перед записью в стор
- [ ] 102. DPR: O(n) среднее → инкрементальная сумма
- [ ] 103. Патрули: `path.shift()` O(n) → индексный курсор
- [ ] 104. Холодные интервалы (useCityNews/useWorldClock/панели) → единый UI-clock
- [ ] 105. Budgets-отчёт: прогнать `check-bundle-budgets --report`, зафиксировать цифры
