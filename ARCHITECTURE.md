...
Kinematic Character Controller (Rapier): capsule r=0.25, autostep 30 см,
snap-to-ground 22 см. `applyImpulsesToDynamicBodies` отключён —
`DynamicProps` (банки/ящики) толкаются через kinematic→dynamic contact
resolution (KCC movement vector applies contact impulse to overlapping
dynamic bodies). Fallback-цепочка:
...
**UI-дыры (P0):** разрешены — см. «Известные разрывы и roadmap» ниже. Escape-роутинг: `escapeDismissAction` + capture-phase `useKeyboardShortcutManager`; inventory z-index: PANEL 60, toasts скрываются при открытых панелях. [roadmap:DOC-02]
