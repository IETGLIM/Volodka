...
- **Радиальное меню E**: только осмысленные действия для объекта (в т.ч. без «Использовать», если для меша нет обработчика в сцене); в шапке — `**itemId**` привязки; после «Взять» — тост с человекочитаемым именем из каталога предметов.
