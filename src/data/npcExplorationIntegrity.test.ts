...
describe('3D / exploration data integrity', () => {
  it('defines explore_mode and explore_hub_welcome aligned with default 3D start (Volodka room)', () => {
    const hub = STORY_NODES.explore_mode;
    expect(hub).toBeDefined();
    expect(hub?.id).toBe('explore_mode');
    expect(hub?.scene).toBe('volodka_room');
    const welcome = STORY_NODES.explore_hub_welcome;
    expect(welcome?.scene).toBe('volodka_room');
  });
