...
export const NPC_DEFINITIONS: Record<string, NPCDefinition> = {
  ...
  cafe_barista: {
    ...
    dialogueTree: {
      id: 'barista_greeting',
      text: 'Добро пожаловать в "Синий кот"! Чем могу помочь? У нас сегодня особенный вечер поэзии.',
      choices: [
        {
...
export const DIALOGUE_NODES: Record<string, NPCDefinition['dialogueTree']> = {
  ...
  barista_about: {
    id: 'barista_about',
    text: '"Синий кот" — место для творческих людей. Поэтов, музыкантов... Каждую пятницу — открытый микрофон.',
    choices: [
      {
  ...
  college_girl_about: {
    ...
    text: '📚 Это "Синий кот". Легендарное место для творческих людей. Тут выступали известные поэты... и неизвестные тоже.',
    effect: { creativity: 5 },
  },
  college_girl_poems: {
