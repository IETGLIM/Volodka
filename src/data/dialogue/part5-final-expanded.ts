...
export const DIALOGUE_PART5_EXPANDED: Record<string, DialogueNode> = {
  ...
  /* ═══════════════════════════════════════════════════════════
     АЛЕКСАНДР — Рефлексия о смене роли (5 новых нод)
     NOTE: ранее этот узел назывался `alexander_redemption` и
     конфликтовал с тёзкой из part3-mid.ts (там — настоящее искупление
     с collectPoem:poem_18 + setFlag alexander_fully_redeemed). При
     мердже part3 перед part5-exp часть5 молча переопределяла часть3,
     теряя стих и флаг. Переименовано здесь во избежание коллизии.
     ═══════════════════════════════════════════════════════════ */

  alexander_role_reflection: {
    id: 'alexander_role_reflection',
    speaker: 'Александр',
