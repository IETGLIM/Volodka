 ...
 *  `returnDialogueNodeId` вместо основного диалога. Ранее 34 NPC ссылались
 *  на несуществующие узлы вида `<npcId>_return` — это вызывало ошибку
 *  «Не удалось загрузить диалог» при повторном разговоре.
 *
...
function mkReturn(
  ...
  return {
    ...
    choices: [
      ...
      { text: 'Расскажи что-нибудь новое.', next: id.replace('_return', '_dialogue') },
    ],
  };
}
...
export const RETURN_DIALOGUE_NODES: Record<string, DialogueNode> = {
  ...
  solnysh_return: mkReturn(
    'solnysh_return',
    'Солныш (Алина)',
    'Ты пришёл... я заждалась. Солнце уже садится, а я всё думаю о тебе. Расскажи мне что-нибудь. Что угодно.',
  ),
