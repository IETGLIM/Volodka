...
export const STORY_NODES_ACT1: Record<string, StoryNode> = {
  ...
  room_table: {
    ...
    speaker: 'narrator',
    sceneId: 'volodka_room',
    effects: [{ type: 'collectPoem', poemId: 'poem_34' }],
    choices: [
      {
  ...
  solnysh_corridor_talk: {
    ...
    text: '«Володька,» — тихо зовёт она, оборачиваясь от зеркала. Голубые глаза усталые, но тёплые. «Не забывай есть, ладно?» Умка крутится у её ног. «Мы с детства рядом… а сегодня мне кажется, что мир слишком серый. Ты меня понимаешь?»',
    speaker: 'Солныш',
    sceneId: 'volodka_corridor',
    choices: [
      {
  ...
  kitchen_table: {
    ...
    guidanceObjectiveType: 'talk_to_npc',
    effects: [{ type: 'collectPoem', poemId: 'poem_35' }],
    choices: [
      {
  ...
  street_bench: {
    ...
    effects: [{ type: 'collectPoem', poemId: 'poem_30' }],
    choices: [
  ...
  fix_success: {
    ...
    speaker: 'narrator',
    sceneId: 'office_day',
    effects: [{ type: 'collectPoem', poemId: 'poem_32' }],
    choices: [
      {
  ...
  balcony_thought: {
    ...
    speaker: 'narrator',
    sceneId: 'home_evening',
    effects: [{ type: 'collectPoem', poemId: 'poem_31' }],
    choices: [
      {
