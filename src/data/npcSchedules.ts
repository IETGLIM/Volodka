...
/**
 * Марат (эхо) — цифровой след первого поэта-прошивщика.
 * [roadmap:ANIM-04] Previously the only NPC without a schedule. Manifests at
 * the library terminal where his echo persists; activity 'work' drives the
 * talk animation via npcActivityAnimation mapping when the player interacts.
 */
const MARAT_ECHO_SCHEDULE: NPCSchedule = {
  id: 'schedule_marat_echo',
  npcId: 'marat_echo',
  entries: [
    { startHour: 0, endHour: 24, sceneId: 'library_day', position: [0, 0, -2], activity: 'work' },
  ],
};
