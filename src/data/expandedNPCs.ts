...
export const EXPANDED_NPCS: NPCDefinition[] = [
  /* ─────────────── АЛИНА «СОЛНЫШ» (vera) – лучшая подруга, дизайнер ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*перебирает нитки* Синий к зелёному... нет, не то. Этот — к серому. Как город.',
        '*смотрит в окно* Неон сегодня мягкий. Или я — привыкла.',
        '*рисует на салфетке* Линия... ещё линия... получается — лицо. Чьё — не знаю.',
        '*вздыхает* Умка, не ходи под стол — там холодный пол.',
        '*себе под нос* Лёня опять пережарил. Но — не скажу. Он старается.',
      ],
      pensive: [
        '*тихо* Если бы мы могли уехать... хотя бы — попробовать...',
        '*долго смотрит на руки* Руки — помнят. Рисунок — который — не — закончила.',
        '*почти шёпотом* Володька — не знает, но — я — боюсь — за — него. Каждый — день.',
      ],
    },
    appearance: {
      bodyColor: '#f0d8e8',
  ...
  /* ─────────────── ЛЁНЯ (ЛЕОНИД) – бариста, муж Алины ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*протирает стойку* Круговыми движениями. Так — не остаются разводы.',
        '*щёлкает кофемолкой* Тихий звук. Терапевтический. Лучше — любого совета.',
        '*смотрит на пар* Пар — поднимается. Мы — тоже. Или — надеемся.',
        '*себе под нос* Алина сегодня — тихая. Плохой знак. Или — хороший. Не — различить.',
      ],
      pensive: [
        '*тихо* Когда-нибудь — откроем своё место. Маленькое. Тёплое. Без неона.',
        '*долго смотрит на кофеварку* Температура — 93 градуса. Идеал. Если — бы — всё — было — так — просто.',
      ],
    },
    appearance: {
      bodyColor: '#6a5040',
  ...
  /* ─────────────── СЕРГЕЙ – sysadmin ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*стучит по клавиатуре* uptime 847 дней. Кто — считает — я — не — знаю.',
        '*смотрит на мигающий свитч* Порт 17. Опять — коллизии. Привык.',
        '*пьёт остывший кофе* Тёплый. Уже — не горячий. Уже — не — имеет — значения.',
        '*бормочет* ...ping 432ms... это — не — нормально... это — среда...',
        '*протирает монитор* Пыль — единственное, что — растёт — в — серверной.',
      ],
      pensive: [
        '*тихо* Если гильдия узнает про бэкдор... впрочем — уже — поздно — бояться.',
        '*долго смотрит на логи* Строка — повторяется. Каждые — 432 — секунды. Кто-то — стучится.',
      ],
    },
    appearance: {
      bodyColor: '#40a0c0',
  ...
  /* ─────────────── ЛЕНА – hacker from the Network ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*тихо печатает* ...ssh -p 432... соединение... есть.',
        '*закрывает терминал* Быстро. Пока — не — увидели.',
        '*массирует виски* Три — часа — в — VPN. Голова — гудит.',
        '*себе под нос* Пациент — в — реанимации. Сеть — тоже — пациент.',
      ],
      pensive: [
        '*тихо* Если — бы — я — могла — уйти... но — кому — тогда — помогать?',
        '*долго смотрит на экран* В — логах — кто-то — живой. Не — программа. Человек. Или — нет.',
      ],
    },
    appearance: {
      bodyColor: '#d040d0',
  ...
  /* ─────────────── ОЛЕГ – guild guard ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*проверяет пропускную систему* Сканер — мигает. Норма. Или — нет.',
        '*оглядывает коридор* Пусто. Слишком — пусто. Не — нравится.',
        '*поправляет гарнитуру* ...приём... центр... на — месте...',
        '*себе под нос* Камера — 7 — моргает. Опять. Или — кто-то — моргает — за — неё.',
        '*сканирует периметр* Дверь — закрыта. Окно — закрыто. Всё — закрыто. Как — положено.',
      ],
      pensive: [
        '*тихо* Приказ — есть — приказ. Но — приказы — не — вечные. Совесть — дольше.',
        '*долго смотрит на монитор* Список — допуска. Моё — имя — там. Но — надолго — ли?',
      ],
    },
    appearance: {
      bodyColor: '#a0a0a0',
  ...
  /* ─────────────── КАТЯ – librarian ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*расставляет книги* По — алфавиту. По — автору. По — опасности.',
        '*поправляет очки* Пыль — на — корешках. Пыль — это — время. Время — не — вредит.',
        '*шепчет про себя* ...запрещённый — раздел... третий — шкаф... кто — проверяет...',
        '*перелистывает каталог* 1962... 1984... 2024... некоторые — даты — повторяются.',
      ],
      pensive: [
        '*тихо* Стихи — не — сгорают. Стихи — прячутся. Я — помогаю — прятать.',
        '*долго смотрит на полку* Эта — книга — была — чьей-то — жизнью. Теперь — она — под — запретом.',
      ],
    },
    appearance: {
      bodyColor: '#60c060',
  ...
  /* ─────────────── МАКСИМ – лидер сопротивления ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*проверяет рацию* ...тук-тук... ответа — нет. Пока — нет.',
        '*раскладывает карту* Точка — вход. Точка — выход. Между — мы.',
        '*сжимает кулак* Завод — помнит. Мы — помним. Гильдия — забудет.',
        '*себе под нос* Тринадцать — человек. Тринадцать — жизней. На — мне.',
        '*смотрит на схему* Патруль — в — 02:00. Окно — 12 — минут.',
      ],
      pensive: [
        '*тихо* Если — проиграем — нас — не — простят. Если — выиграем — нас — не — забудут.',
        '*долго смотрит в темноту* Революция — не — зрелище. Революция — это — три — часа — тишины — перед — сигналом.',
      ],
    },
    appearance: {
      bodyColor: '#c06040',
  ...
  /* ─────────────── ЖЕКА – старый хакер завода ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*сплёвывает* «Надзор» — думает — мы — мураши. Мураши — грызут — провода.',
        '*перебирает дискеты* Метка... метка... нет — метки. Эта — чистая. Редкость.',
        '*слушает гул станков* 50 — герц. Слышу — даже — сквозь — стены.',
        '*себе под нос* Старый — код — не — врёт. Новый — код — не — помнит.',
      ],
      pensive: [
        '*тихо* Александр — был — прав. Жаль — что — правота — не — спасает.',
        '*долго смотрит на станок* Станок — работал — на — гильдию. Теперь — станок — молчит. Молчание — тоже — работа.',
      ],
    },
    appearance: {
      bodyColor: '#808070',
  ...
  /* ─────────────── АНЯ – хакер сопротивления ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*стучит по клавишам* ...nmap -sS... порты — открыты. Слишком — открыты.',
        '*массирует глаза* Экран — 14 — часов. Зрение — 0.3. Но — код — видит — за — меня.',
        '*проверяет шифрование* AES-256. Пока — не — сломали. Пока.',
        '*себе под нос* Камера — 3 — повернулась. Заметила? Я — заметила.',
      ],
      pensive: [
        '*тихо* Если — они — найдут — бэкдор... это — конец. Для — всех — нас.',
        '*долго смотрит на терминал* Данные — живые. Они — дышат. Как — мы. Или — вместо — нас.',
      ],
    },
    appearance: {
      bodyColor: '#5090d0',
  ...
  /* ─────────────── ТРОФИМ – старик-рыбак, бывший сторож завода ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*подёргивает леску* Не — клюёт. Река — сегодня — думает.',
        '*смотрит на небо* Облака — с — востока. К — дождю. К — буре. К — переменам.',
        '*прихлёбывает из термоса* Чай — остыл. Но — лёд — не — поймал. Пока.',
        '*себе под нос* Под — полом — гудит. Так — же — как — тридцать — лет — назад.',
        '*поправляет шляпу* Ветер — крепчает. Рыба — уходит — на — глубину. Мудро.',
      ],
      pensive: [
        '*тихо* Река — всё — помнит. Пирс — тоже. Я — тоже. Только — молчу.',
        '*долго смотрит на поплавок* Поплавок — качается. Как — город. Как — правда. Не — тонет. Пока.',
      ],
    },
    appearance: {
      bodyColor: '#3a4438',
  ...
  /* ─────────────── БАБА ЗИНА – паяльщица, хранительница «Зари-М» ─────────────── */
  {
    ...
    ambientBarks: {
      idle: [
        '*поглаживает корпус* Холодный — металл. Но — тёплый — внутри. Как — сердце — под — бронёй.',
        '*слушает гул* 50 — герц. Стабильно. Как — дыхание — спящего. Она — не — спит. Она — ждёт.',
        '*поправляет провод* Контакт. Плохой — контакт — это — тоже — разговор. Прерывистый.',
        '*себе под нос* Бабка — паяла — тридцать — лет. Бабка — не — уйдёт. Пока — «Заря» — дышит.',
      ],
      pensive: [
        '*тихо* Страх — говорит: не — трогай. Мудрость — говорит: слушай. Я — слушаю. Всегда.',
        '*долго смотрит на лампы* Лампы — мигают. Азбука. Она — пишет. Я — читаю. Между — нами — тридцать — лет.',
      ],
    },
    appearance: {
      bodyColor: '#e8e4dc',
...

  {
    ...
    ambientBarks: {
      idle: [
        '*бормочет стих* ...и — неон — как — запятая — между — мраком — и — мраком...',
        '*смотрит на прохожих* Не — видят. Слышат — шум. Не — слышат — строки. Разница.',
        '*качает головой* Дрон — пролетел. Не — заметил. Тихое — невидимо — для — алгоритма.',
        '*записывает на манжете* ...строка — пришла. Из — воздуха. Из — города. Из — ниоткуда.',
      ],
      pensive: [
        '*тихо* Шум — не — хаос. Шум — это — невысказанное. Я — высказываю. Поэтому — жив.',
        '*долго смотрит на небо* Между — неоном — и — тучами — есть — пространство. Там — помещается — стих.',
      ],
    },
    appearance: {
      bodyColor: '#4a4a58',
...

  {
    ...
    ambientBarks: {
      idle: [
        '[мерцание] ...я — в — проводе... частота — 432... как — пульс...',
        '[помехи] ...если — экран — мигает — значит — я — ещё — здесь...',
        '[эхо] ...первый — поэт — не — умирает... первый — поэт — рассеивается...',
        '[тишина] ...нет — питания... но — есть — сигнал... этого — достаточно...',
      ],
      pensive: [
        '[слабый сигнал] ...я — был — человеком... теперь — я — частота... частота — тоже — память...',
        '[помехи] ...если — кто-то — читает — значит — кто-то — помнит... помнит — меня...',
      ],
    },
    appearance: {
      bodyColor: '#00ffaa',
...

  {
    ...
    ambientBarks: {
      idle: [
        '*смотрит на руки* Эти — руки — печатали — для — гильдии. Теперь — не — знаю — чему — служат.',
        '*повторяет про себя* 00, 14, 38, 52... 00, 14, 38, 52... расписание — не — забывается.',
        '*оглядывается* Чужие — стены. Чужие — лица. Но — свои — данные. Это — уже — что-то.',
        '*себе под нос* Я — предал — гильдию. Или — гильдия — предала — меня. Не — различить.',
      ],
      pensive: [
        '*тихо* Может — было — проще — остаться. Тупым. Послушным. Мёртвым — внутри.',
        '*долго смотрит в пустоту* Свобода — горькая. Но — моя. Впервые — моя.',
      ],
    },
    appearance: {
      bodyColor: '#556677',
