...
/**
 * Роль инвентаря (фаза B): не «шоп RPG», а **носители смысла и механики** —
 * сюжетные ключи и документы, фрагменты стихов, подарки/воспоминания, редкие расходники (энергия/статы).
 * Стихи как контент живут в `collectPoem`; сюжетные ворота — во флагах/квестах; предмет — то, что игрок *держит в руке*
 * (в т.ч. для `hasItem` / подсказок в обходе). Лишние id не вводим: каждый `itemId` в сценах должен существовать здесь.
 */
// ============================================
// ПРЕДМЕТЫ ИГРЫ
// ============================================
...
export const items: Record<string, Item> = {
  ...
  maria_poem: {
    ...
    description: 'Распечатанное стихотворение, которое Виктория подарила тебе.',
    type: 'key',
    rarity: 'rare',
    icon: '📝',
    droppable: false,
  ...
  memory_maria_smile: {
    ...
    description: 'Момент, когда Виктория впервые по-настоящему улыбнулась тебе.',
    type: 'memory',
    rarity: 'rare',
    icon: '😊',
    droppable: false,
  ...
  poem_fragment_2: {
    id: 'poem_fragment_2',
    name: 'Второй обрывок стиха',
    description: 'Страница из чьей-то тетради — другой почерк, те же ночные кафе.',
    type: 'document',
    rarity: 'uncommon',
    icon: '📄',
    droppable: true,
    stackable: false,
    maxStack: 1,
    lore: 'На полях карандашом: «продолжение следует» — и дата, которую ты уже видел в другом месте.',
  },
