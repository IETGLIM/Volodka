...
export function runMainPlayerMovement(deps: PlayerMovementDeps): boolean {
  ...
  // [roadmap:PLYR-07] Jump buffer — if jump pressed while airborne, store the
  // intent for JUMP_BUFFER_TIME seconds. When the player lands (isGrounded
  // becomes true) within that window, the jump fires immediately. Without
  // buffering, a jump press 50ms before landing is lost — feels unresponsive.
  // Standard since ~2010 (Valve L4D, Hollow Knight, Celeste).
  if (jumping) {
    deps.jumpBufferTimerRef.current = JUMP_BUFFER_TIME;
  } else if (deps.jumpBufferTimerRef.current > 0) {
    deps.jumpBufferTimerRef.current = Math.max(0, deps.jumpBufferTimerRef.current - dt);
  }

  const hasJumpBuffered = deps.jumpBufferTimerRef.current > 0;
  const wantsJump =
    hasJumpBuffered &&
  ...
  if (wantsJump && !deps.controlsDegradedRef.current) {
    ...
    deps.coyoteTimerRef.current = 0;
    // Consume the buffer on successful jump — prevents double-jump from a
    // single buffered press.
    deps.jumpBufferTimerRef.current = 0;
  } else if (
    !airborneIntent &&
