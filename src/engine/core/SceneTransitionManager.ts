...
import { scheduleSceneLoaded } from './sceneLoadedGate';
import {
...
/**
 * [roadmap:ARCH-03] Import-time binder calls removed — all 4 binders are now
 * invoked from `reviveGameEngine()` (initial boot + StrictMode/HMR remount).
 * This prevents `eventBus.on(...)` throwing via `assertSubscribable` if the
 * module is imported before the EventBus is revived.
 */
