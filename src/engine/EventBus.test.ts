...
describe('EventBusClass lifecycle', () => {
  ...
  /**
   * [roadmap:ARCH-18] In dev mode, subscribing on a disposed bus auto-revives
   * (with warning) instead of throwing — prevents HMR crash when modules hold
   * stale references. In production, still throws.
   */
  it('auto-revives on subscribe in dev mode (HMR safety)', () => {
    const bus = createEventBus();
    bus.dispose();
    // Dev mode: auto-revive, no throw
    expect(() => bus.on('sound:play', vi.fn())).not.toThrow();
    expect(() => bus.onAny(vi.fn())).not.toThrow();
    // Bus is revived after the auto-revive
