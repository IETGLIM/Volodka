 ...
 *
 * This function is called per-frame via `registerFrameTick('weather', ...)`
 * on wet-street scenes (see StreetVisual.tsx, useWetSurfaceMaterial.ts).
 * It must NOT set `material.needsUpdate = true`: `roughness` and `metalness`
 * are shader uniforms that Three.js uploads automatically on the next render
 * via `refreshUniformsCommon`. Setting `needsUpdate` forces a full shader
 * program recompile every frame, which is catastrophic for FPS on wet-street
 * scenes.
 *
 * [roadmap:P0-GFX-01] Removed per-frame `needsUpdate = true` that caused
 * shader recompilation on every frame in wet-street scenes.
 */
