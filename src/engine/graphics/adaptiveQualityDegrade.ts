...
export function degradeQualityPresetOneTier(): QualityPresetId | null {
  ...
  if (current === 'auto') {
    ...
    dispatchQualityGpuCleanup('auto', 'degrade');
  ...
  // [roadmap:GFX-04] Surgical eviction on degrade — texture caches only.
  dispatchQualityGpuCleanup(next, 'degrade');
...
export function upgradeQualityPresetOneTier(): QualityPresetId | null {
  ...
  if (current === 'auto') {
    ...
    const next = QUALITY_PRESET_ORDER[idx + 1];
    setSessionAutoResolvedTier(next);
    // [roadmap:GFX-04] Full eviction on upgrade — new tier may use different GLB variants.
    dispatchQualityGpuCleanup('auto', 'upgrade');
    dispatchQualityPresetChanged('auto', { autoRuntimeTier: next });
    return 'auto';
  }
  ...
  writeQualityPresetId(next);
  // [roadmap:GFX-04] Full eviction on upgrade — new tier may use different GLB variants.
  dispatchQualityGpuCleanup(next, 'upgrade');
  dispatchQualityPresetChanged(next);
  return next;
}
