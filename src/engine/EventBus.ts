  ...
  implements EventBusScopeHost<TMap>
{
  ...
  /**
   * [roadmap:ARCH-18] Previously threw if the bus was disposed. This trapped
   * HMR: registerHmrDispose(disposeEventBus) sets disposed=true on hot reload,
   * but modules holding the old singleton reference would throw on next
   * subscribe. Now auto-revives in dev mode (with a warning) instead of
   * throwing — prevents HMR crash without hiding the issue.
   * In production, still throws (disposed bus = real bug).
   */
  private assertSubscribable(operation: 'on' | 'onAny'): void {
    if (!this.disposed) return;
    if (import.meta.env.DEV) {
      console.warn(
        `[EventBus] Auto-reviving disposed bus for ${operation}() — ` +
          'this usually means a module held a stale reference across HMR. ' +
          'Call reviveEventBus() explicitly for cleaner lifecycle.',
      );
      this.revive();
      return;
    }
