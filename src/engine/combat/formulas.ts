...
export const COMBAT_CONSTANTS = {
  ...
  /** [roadmap:P0-CMBT-01] nexus_guardian (Act 6 boss) special-attack multipliers */
  NEXUS_PURGE_DAMAGE_MULTIPLIER: 2,
  NEXUS_PURGE_DEFENSE_REDUCTION: 0.35,
  NEXUS_PROTOCOL_DEFENSE_BOOST: 10,
  NEXUS_PROTOCOL_ATTACK_BOOST: 6,
  NEXUS_OVERSIGHT_LOGIC_DRAIN: 3,
  /** [roadmap:P0-CMBT-02] void_echo special-attack multipliers */
  VOID_TEAR_DAMAGE_MULTIPLIER: 1.8,
  VOID_TEAR_HP_DRAIN_PERCENT: 0.1,
  VOID_CALL_DEFENSE_REDUCTION: 0.25,
  VOID_CALL_EMPATHY_DRAIN: 3,
  /**
   * [roadmap:STRS-01] Stress applied by combat events.
   * Darkest Dungeon-pattern: combat is a stress source, not just an HP sink.
   * - ENEMY_HIT_STRESS_PER_MAX_HP_FRACTION: stress per hit, scaled by damage/maxHp
   *   (big hits hurt more — a 20 HP hit on 50 maxHp = 4 stress; a 5 HP hit = 1 stress)
   * - ENEMY_HIT_STRESS_MIN/MAX: clamp so tiny taps still sting and big nukes don't break
   * - VICTORY_STRESS_RELIEF: small relief after winning (release of tension)
   * - DEFEAT_STRESS_PENALTY: shock of defeat (large stress spike)
   */
  ENEMY_HIT_STRESS_PER_MAX_HP_FRACTION: 10,
  ENEMY_HIT_STRESS_MIN: 1,
  ENEMY_HIT_STRESS_MAX: 6,
  VICTORY_STRESS_RELIEF: -5,
  DEFEAT_STRESS_PENALTY: 10,
  /**
   * [roadmap:STRS-02] Stress > 80 defense penalty (trade-off).
   * Existing perks give bonuses at high stress (code_rage +4 attack at >60,
   * stress_mastery +15% damage at >50). This adds the downside — at >80 stress
   * the player is too rattled to defend effectively. Small penalty (-2 flat
   * defense) so the trade-off is meaningful but not punishing.
   */
  HIGH_STRESS_THRESHOLD: 80,
  HIGH_STRESS_DEFENSE_PENALTY: 2,
} as const;
...
export function getPlayerDefense(): number {
  const s = snap();
  const { skills, energy, stress } = s.playerState;
  ...
  // [roadmap:STRS-02] High-stress defense penalty (trade-off).
  // Existing perks give bonuses at high stress (code_rage +4 attack at >60,
  // stress_mastery +15% damage at >50). This adds the downside: at >80 stress
  // the player is too rattled to defend effectively. Small flat penalty so
  // the trade-off is meaningful but not punishing (min floor enforced below).
  const highStressPenalty =
    stress > COMBAT_CONSTANTS.HIGH_STRESS_THRESHOLD
      ? COMBAT_CONSTANTS.HIGH_STRESS_DEFENSE_PENALTY
      : 0;
  return Math.max(
    0,
    Math.floor(base * perks.defenseMultiplier) + perks.flatDefenseBonus - highStressPenalty,
  );
}
