...
/**
 * Хранитель «Надзора» — Act 6 boss.
 * [roadmap:P0-CMBT-01] Previously had empty specialAttacks: [] (shipping blocker).
 * Three specials: oversight debuff, defense protocol self-buff, memory purge nuke.
 */
const NEXUS_GUARDIAN_SPECIALS: EnemySpecialAttack[] = [
  {
    id: 'nexus_total_oversight',
    name: 'Тотальный Надзор',
    description: 'Сканирует уязвимости: -логика 3, защита снижена на 2 хода',
    chance: 0.3,
    cooldown: 3,
    execute: (state, enemy) => {
      const buff1 = createBuff(
        state,
        'Тотальный Надзор',
        'nexus_oversight_vuln',
        'debuff',
        'player',
        2,
        { type: 'defense_reduction', value: COMBAT_CONSTANTS.NEXUS_PURGE_DEFENSE_REDUCTION },
      );
      let s = addBuff(state, buff1);
      const buff2 = createBuff(
        s,
        'Надзор: Логика',
        'nexus_oversight_logic',
        'debuff',
        'player',
        2,
        { type: 'stat_drain', stat: 'logic', value: COMBAT_CONSTANTS.NEXUS_OVERSIGHT_LOGIC_DRAIN },
      );
      s = addBuff(s, buff2);
      return {
        ...s,
        log: [
          ...
          {
            turn: state.turn,
            text: `${enemy.emoji} Тотальный Надзор! Под прицелом: логика -${COMBAT_CONSTANTS.NEXUS_OVERSIGHT_LOGIC_DRAIN}, защита снижена!`,
            type: 'enemy_special' as const,
          },
        ],
      };
  ...
  {
    id: 'nexus_defense_protocol',
    name: 'Защитный Протокол',
    ...
    execute: (state, enemy) => {
      const buff1 = createBuff(
        ...
        'enemy',
        2,
        { type: 'defense_boost', value: COMBAT_CONSTANTS.NEXUS_PROTOCOL_DEFENSE_BOOST },
      );
      let s = addBuff(state, buff1);
      const buff2 = createBuff(
        s,
        ...
        'enemy',
        2,
        { type: 'attack_boost', value: COMBAT_CONSTANTS.NEXUS_PROTOCOL_ATTACK_BOOST },
      );
      s = addBuff(s, buff2);
      return {
        ...s,
        log: [
          ...
          {
            turn: state.turn,
            text: `${enemy.emoji} Защитный Протокол! Защита +${COMBAT_CONSTANTS.NEXUS_PROTOCOL_DEFENSE_BOOST}, атака +${COMBAT_CONSTANTS.NEXUS_PROTOCOL_ATTACK_BOOST}!`,
            type: 'enemy_special' as const,
          },
        ],
      };
  ...
  {
    id: 'nexus_memory_purge',
    name: 'Очистка Памяти',
    description: 'Тяжёлый удар (2x) + стирает все баффы игрока',
    chance: 0.25,
    cooldown: 5,
    execute: (state, enemy) => {
      const effectiveAttack = enemy.attack + getEnemyAttackBoost(state);
      ...
      let damage = rolled.damage * COMBAT_CONSTANTS.NEXUS_PURGE_DAMAGE_MULTIPLIER;
      const nextState = rolled.state;

      const playerDefBoost = getPlayerDefenseBoost(nextState);
      ...
      const newPlayerHp = Math.max(0, nextState.playerHp - damage);
      const purgedCount = nextState.buffs.filter((b) => b.target === 'player').length;
      const remainingBuffs = nextState.buffs.filter((b) => b.target !== 'player');
      return {
        ...nextState,
        playerHp: newPlayerHp,
        ...
        log: [
          ...
          {
            turn: state.turn,
            text: `${enemy.emoji} Очистка Памяти! -${damage} HP! ${purgedCount > 0 ? `Стерто ${purgedCount} ваших усилений!` : 'Ваши мысли стерты.'}`,
            type: 'enemy_special' as const,
            damage,
          },
        ],
...
/**
 * Эхо Пустоты — Act 6 void entity.
 * [roadmap:P0-CMBT-02] Previously had empty specialAttacks: [] (shipping blocker).
 * Two specials: call of the void (debuff), reality tear (nuke + DoT).
 */
const VOID_ECHO_SPECIALS: EnemySpecialAttack[] = [
  {
    id: 'void_call_of_the_void',
    name: 'Зов Пустоты',
    description: 'Истощает эмпатию 3 + снижает защиту на 2 хода',
    chance: 0.35,
    cooldown: 3,
    execute: (state, enemy) => {
      const buff1 = createBuff(
        ...
        'player',
        2,
        { type: 'defense_reduction', value: COMBAT_CONSTANTS.VOID_CALL_DEFENSE_REDUCTION },
      );
      let s = addBuff(state, buff1);
      const buff2 = createBuff(
        s,
        ...
        'player',
        2,
        { type: 'stat_drain', stat: 'karma', value: COMBAT_CONSTANTS.VOID_CALL_EMPATHY_DRAIN },
      );
      s = addBuff(s, buff2);
      return {
        ...s,
        log: [
          ...
          {
            turn: state.turn,
            text: `${enemy.emoji} Зов Пустоты! Эмпатия истощена, защита снижена!`,
            type: 'enemy_special' as const,
          },
        ],
      };
  ...
  {
    id: 'void_reality_tear',
    name: 'Разрыв Реальности',
    description: 'Стремительный удар (1.8x) + заражает: -10% HP/ход, 2 хода',
    chance: 0.3,
    cooldown: 4,
    execute: (state, enemy) => {
      const effectiveAttack = enemy.attack + getEnemyAttackBoost(state);
      ...
      let damage = rolled.damage * COMBAT_CONSTANTS.VOID_TEAR_DAMAGE_MULTIPLIER;
      const nextState = rolled.state;
      ...
      const drainBuff = createBuff(
        ...
        'player',
        2,
        { type: 'hp_drain_percent', value: COMBAT_CONSTANTS.VOID_TEAR_HP_DRAIN_PERCENT },
      );
      const s = addBuff({ ...nextState, playerHp: newPlayerHp }, drainBuff);
      return {
        ...s,
        log: [
          ...s.log,
          {
            turn: state.turn,
            text: `${enemy.emoji} Разрыв Реальности! -${damage} HP! Реальность сочится пустотой: -${Math.round(COMBAT_CONSTANTS.VOID_TEAR_HP_DRAIN_PERCENT * 100)}% HP/ход!`,
            type: 'enemy_special' as const,
            damage,
          },
        ],
 ...
 *
 * [roadmap:CMBT-05] Previously swapped enemy type silently — no warning, no
 * telemetry. This broke the narrative contract for scripted encounters (e.g.,
 * Act 6 boss `nexus_guardian` could silently become `system_daemon` if player
 * was under-leveled). Now emits a dev warning and returns the fallback.
 * Callers can pass `allowFallback: false` for scripted boss encounters to
 * force the requested type regardless of phase gates.
 */
export function resolveEnemyType(
  requestedType: EnemyType,
  ...
  // Scripted encounters (boss fights) force the requested type regardless of phase.
  if (!allowFallback) {
    if (import.meta.env.DEV) {
      console.warn(
        `[resolveEnemyType] Scripted encounter forced: ${requestedType} ` +
          `(player L${playerLevel}/Act${currentAct} < required L${unlock.minLevel}/Act${unlock.minAct}). ` +
          `Phase gate bypassed via allowFallback:false.`,
      );
    }
  ...
  const fallback = fallbacks[pickIndexFromSeed(fallbackSeed, fallbacks.length)];

  if (import.meta.env.DEV) {
    console.warn(
      `[resolveEnemyType] Phase fallback: ${requestedType} → ${fallback} ` +
        `(player L${playerLevel}/Act${currentAct} < required L${unlock.minLevel}/Act${unlock.minAct}).`,
    );
  }
  return fallback;
