...
export const ENEMY_AFFINITIES: Record<EnemyType, EnemyAffinityMap> = {
  ...
  ranged_strelkov: {
    ...
    logic: 1.0,      // Логические уязвимости (нейтрально)
  },
  dark_mage: {
    physical: 0.5,   // Маги устойчивы к физическому урону
    code: 1.5,       // Код нарушает их заклинания
    writing: 2.0,    // Поэзия разрушает магические барьеры (суперэффективно!)
    empathy: 0.7,    // Маги холодны к эмпатии
  },
  boss_catacombs_keeper: {
    physical: 0.5,   // Босс устойчив к обычным атакам
    code: 1.0,       // Код-эксплойты нейтральны
    writing: 2.0,    // Стихи — главное оружие против Хранителя
    empathy: 1.5,    // Эмпатия пробивает его броню
    logic: 0.7,      // Хранитель сопротивляется логике
  },
  boss_silent_warden: {
    physical: 0.5,   // Тишина гасит удары
    code: 1.5,       // Код-эксплойты рушат архив прослушки
    writing: 2.0,    // Стихи — главное оружие против тишины
    empathy: 1.0,    // Эмпатия нейтральна
    logic: 0.7,      // Логикой стража прослушки не переиграть
  },
};
