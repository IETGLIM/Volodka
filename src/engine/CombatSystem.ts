...
function consumeSkipTurn(state: CombatState, target: 'player' | 'enemy'): { state: CombatState; consumed: boolean } {
  const consumed = hasBuffEffect(state, target, 'skip_turn');
  if (!consumed) return { state, consumed: false };

  return {
    consumed: true,
    state: {
      ...state,
      buffs: state.buffs.filter((b) => !(b.target === target && b.effect.type === 'skip_turn')),
    },
  };
}
...
export function playerAttack(): CombatState | null {
  ...
  // Enemy turn
  return endPlayerTurn({ skipPowerCooldownTick: true });
}
...
export function playerFlee(): CombatState | null {
  ...
  if (fled) {
    ...
    // Return to story node or exploration after a brief delay
    setTimeout(() => {
      const store = getGameStore();
      if (returnNodeId) {
...
function endPlayerTurn(options?: { skipPowerCooldownTick?: boolean }): CombatState {
  ...
  currentCombat = {
    ...currentCombat,
    isPlayerTurn: false,
    powerCooldowns: options?.skipPowerCooldownTick ? currentCombat.powerCooldowns : tickPowerCooldowns(currentCombat.powerCooldowns),
  };
...
function transitionToPlayerTurn(state: CombatState): void {
  ...
  const skipTurn = consumeSkipTurn(state, 'player');
  const baseState = skipTurn.state;

  // ── Tick player buffs ──
  const { state: afterBuffTick, expiredLog } = tickBuffs(baseState, 'player');
...
function executeEnemyTurn() {
  ...
  const skipTurn = consumeSkipTurn(currentCombat, 'enemy');
  const baseState = skipTurn.state;

  // ── Tick enemy buffs ──
  const { state: afterBuffTick, expiredLog } = tickBuffs(baseState, 'enemy');

  // ── Check if enemy is stunned (skip_turn debuff on enemy) ──
  if (skipTurn.consumed || afterBuffTick.enemyDefending) {
    currentCombat = {
      ...afterBuffTick,
      enemyDefending: false,
      log: [
