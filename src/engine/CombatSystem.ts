...
function startCombatImmediate(
  ...
  // [roadmap:CMBT-05] Scripted boss encounters (nexus_guardian, void_echo)
  // force the requested type — never silently swap to a weaker enemy.
  // These are narrative bosses; swapping breaks the story contract.
  const isScriptedBoss = enemyType === 'nexus_guardian' || enemyType === 'void_echo';
  const resolvedType = isScriptedBoss
    ? resolveEnemyType(enemyType, { allowFallback: false })
    : resolveEnemyType(enemyType);
 ...
 *
 * [roadmap:POEM-04] Also processes expired TTL flags at turn start. Previously
 * TTL expiry only ran on a 1s setInterval in useGameLifecycleManager — flags
 * expiring mid-combat didn't revert until post-combat. Now each turn tick
 * processes expiry, so poem-power buffs (e.g. guiding_star_active,
 * stone_skin_active) revert promptly during long boss fights.
 */
function transitionToPlayerTurn(state: CombatState): void {
  if (!combat.getState()) return;

  // ── Process expired TTL flags (poem-power buffs) ──
...
function executeEnemyTurn() {
  ...
  // [roadmap:STRS-01] Combat applies stress on enemy hit (Darkest Dungeon loop).
  // Stress scales with damage/maxHp fraction — big hits rattle more than glancing taps.
  // Dispatched via player/addStress so scaleStressWithPoemEffects funnel applies
  // (stone_skin_active halves, stress_resist perk reduces, etc.).
  const playerMaxHp = getPlayerMaxHp();
  const stressFraction = playerMaxHp > 0 ? enemyDamage / playerMaxHp : 0;
  const hitStress = Math.min(
    COMBAT_CONSTANTS.ENEMY_HIT_STRESS_MAX,
    Math.max(
      COMBAT_CONSTANTS.ENEMY_HIT_STRESS_MIN,
      Math.round(stressFraction * COMBAT_CONSTANTS.ENEMY_HIT_STRESS_PER_MAX_HP_FRACTION),
    ),
  );
  dispatchGameAction({ type: 'player/addStress', amount: hitStress });
...
function handleVictory(): CombatState {
  ...
  // [roadmap:STRS-01] Victory applies small stress relief (release of tension).
  // Closes the Darkest Dungeon loop: combat → stress → victory → relief.
  dispatchGameAction({ type: 'player/addStress', amount: COMBAT_CONSTANTS.VICTORY_STRESS_RELIEF });
...
function handleDefeat(): void {
  ...
  // [roadmap:STRS-01] Defeat applies large stress penalty (shock of defeat).
  // Pairs with VICTORY_STRESS_RELIEF to make the stress loop bidirectional.
  dispatchGameAction({ type: 'player/addStress', amount: COMBAT_CONSTANTS.DEFEAT_STRESS_PENALTY });
