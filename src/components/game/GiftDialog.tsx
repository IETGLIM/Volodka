...
export function GiftDialog({ open, onClose, npcId }: GiftDialogProps) {
  ...
  /* ── Filter and sort inventory items by gift preference ── */
  const giftableItems = useMemo(() => {
    const items = inventory
      .filter((item) => {
        // Don't show quest items
        const def = getItemDefinition(item.id);
        return !def?.questRelated;
      })
      .map((item) => {
        const pref = getItemPreference(npcId, item.id);
        const change = getAffinityChange(pref);
        return { ...item, preference: pref, affinityChange: change };
      })
      // Sort: loved first, then liked, then neutral, then disliked, then hated
      .sort((a, b) => {
        const order: Record<GiftPreference, number> = { loved: 0, liked: 1, neutral: 2, disliked: 3, hated: 4 };
  ...
  }, [npcId, npcDef, giftItemToNPC]);

  /* ── Affinity bar color based on level ── */
  ...
  // Map affinity (-100 to 100) to bar percentage (0% to 100%)
          ...
          >
                  ...
                  {giftableItems.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`flex items-center justify-center w-8 h-8 rounded-md text-sm shrink-0 ${prefBg}`}
            ...
            <div className="px-4 py-2 border-t border-slate-800/40 bg-black/20">
              <div className="flex items-center justify-between text-[10px] text-slate-500">
                ...
                {giftPrefs && (
                  <div className="flex items-center gap-2">
                    <span className="text-amber-400">❤️ {giftPrefs.lovedItems.length}</span>
                    <span className="text-emerald-400">👍 {giftPrefs.likedItems.length}</span>
                  </div>
                )}
              </div>
            </div>
