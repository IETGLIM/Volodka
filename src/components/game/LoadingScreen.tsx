...
/* ════════════════════════════════════════════════════════════════
   CANVAS MATRIX RAIN — full green katakana/hex rain background
   ════════════════════════════════════════════════════════════════ */
...
const CanvasMatrixRain = memo(function CanvasMatrixRain() {
  ...
  useEffect(() => {
    ...
    const scheduleResize = () => {
      ...
      resizeTimer = setTimeout(() => { resizeTimer = undefined; applyResize(); }, 120);
    };
    ...
    const chars = 'アイウエオカキクケコサシスセソ0123456789ABCDEFsudo_nova_grep_awk';
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 pointer-events-none" style={{ opacity: 0.15 }} />;
});
...
export function LoadingScreen({ progress, message = 'Загрузка...', showTitle = false }: LoadingScreenProps) {
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-black overflow-hidden" style={{ zIndex: UI_LAYERS.LOADING }}>
