...
import { UnifiedNarrativePanel } from './UnifiedNarrativePanel';
      ...
      <>
          ...
          {(mode === 'exploration' || mode === 'visual-novel' || mode === 'cutscene' || mode === 'combat') && (
            <>
              ...
              {/* UNIFIED NARRATIVE PANEL — single cohesive interface for all narrative content
                  Replaces separate StoryRenderer, DialogueRenderer, and ExaminePanel.
                  Provides continuous story feed with history tracking.
              */}
              <ErrorBoundary name="unified-narrative">
                <UnifiedNarrativePanel />
              </ErrorBoundary>

              {/* FALLBACK: Original separate renderers (disabled, kept for reference)
              {!isDialogueActive && (
              ...
              */}
              ...
              {/* Examine Panel — now handled by UnifiedNarrativePanel, kept as fallback */}
              {/*
              <ExaminePanel
                open={examineOpen}
                data={examineData}
