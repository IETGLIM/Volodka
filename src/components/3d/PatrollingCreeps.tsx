 ...
 *  past dangers: creep vision shrinks while active.
 *  [roadmap:STLTH-03] Vision scale now read from POEM_TTL_CONSUMERS registry
 *  (creepVisionScale field) instead of hardcoded constant — makes stealth
 *  systemically responsive to ALL poem effects with creepVisionScale, not
 *  just guiding_star. */
const GUIDING_STAR_FLAG = 'guiding_star_active';
/** Fallback when registry entry is missing or field undefined. */
      ...
      frameCtxRef.current = {
        ...
        poemVisionScale:
          flag && isActiveTTLFlagLive({ [flag.key]: flag }, flag.key)
            ? // [roadmap:STLTH-03] Read from registry — allows ANY poem TTL flag
              // with creepVisionScale to affect stealth, not just guiding_star.
              (getPoemTTLConsumer(flag.key)?.creepVisionScale ?? DEFAULT_VISION_SCALE)
