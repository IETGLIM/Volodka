...
export function VolodkaRoomVisual({ livePlayerPositionRef: _livePlayerPositionRef }: VolodkaRoomVisualProps) {
  ...
  return (
    <group>
      ...
      {/* ── Coat on hook by door (front wall area) ── */}
      ...
      {/* ── Small plant in pot on floor near window ── */}
      ...
      {/* Grease stain on pizza box */}
      <mesh position={[0.55, 0.795, -2.3]} rotation={[0, 0.3, 0]}>
        <circleGeometry args={[0.06, 8]} />
        <meshStandardMaterial color="#a08030" roughness={0.95} transparent opacity={0.6} polygonOffset polygonOffsetFactor={-1} polygonOffsetUnits={-1} />
      </mesh>

      {/* ── Energy drink cans on floor (2 cans) ── */}
