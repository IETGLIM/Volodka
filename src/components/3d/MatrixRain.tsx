...
export function MatrixRain({ sceneId: sceneIdProp }: MatrixRainProps) {
  ...
  const enabled = isCutsceneOrStory || explorationPreset !== null;

  // Active preset: during cutscene/story use DEFAULT_PRESET, during exploration use scene preset
  const activePreset = isCutsceneOrStory ? DEFAULT_PRESET : (explorationPreset ?? DEFAULT_PRESET);
