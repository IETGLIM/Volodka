 ...
 */
import { BoxGeometry, BufferAttribute, BufferGeometry, CapsuleGeometry, CircleGeometry, Color, ConeGeometry, CylinderGeometry, Group, Mesh, MeshBasicMaterial, MeshStandardMaterial, OctahedronGeometry, SphereGeometry, Vector3 } from 'three';
...
function soft(geo: BufferGeometry, amt: number) {
  return deformGeometry(geo, amt, 1.3, rand(0, 200));
}
function softSphere(r: number, seg = 28) {
  return soft(new SphereGeometry(r, seg, seg * 0.75), r * 0.06);
}
function softCapsule(r: number, h: number, seg = 20) {
  return soft(new CapsuleGeometry(r, h, seg, seg + 4), r * 0.07);
}

function mat(color: string, rough = 0.8, em?: string, ei = 0) {
  const m = new MeshStandardMaterial({ color, roughness: rough });
...
export class Npc {
  ...
  constructor(kind: NpcKind, home: V3, radius: number, heightAt: (x: number, z: number) => number) {
    ...
    this.target.copy(new Vector3(home[0], 0, home[2]));
    this.wait = rand(0, 3);
  ...
  private buildStarets() {
    ...
    // Тело — мантия
    const bodyMesh = new Mesh(softCapsule(0.4, 0.8, 28), robe);
    bodyMesh.position.y = 0.72;
    bodyMesh.castShadow = true;
    this.body.add(bodyMesh);

    // Плечи
    const shoulders = new Mesh(softSphere(0.36, 28), cloak);
    ...
    const mkArm = (side: number) => {
      ...
      const upper = new Mesh(softCapsule(0.09, 0.32, 18), robe);
      upper.position.y = -0.2;
      g.add(upper);
      const elbow = new Mesh(softSphere(0.08, 16), robe);
      elbow.position.y = -0.4;
      g.add(elbow);
      const forearm = new Mesh(softCapsule(0.08, 0.28, 16), robe);
      forearm.position.y = -0.58;
      g.add(forearm);
      const hand = new Mesh(softSphere(0.065, 16), skin);
    ...
    // Голова
    const faceGeo = soft(new SphereGeometry(0.2, 30, 26), 0.018);
    ...
    // Борода длинная
    const beardGeo = soft(new ConeGeometry(0.18, 0.55, 20), 0.025);
    const beardMesh = new Mesh(beardGeo, beard);
    ...
    for (const s of [-1, 1]) {
      const b = new Mesh(
        soft(new CapsuleGeometry(0.015, 0.065, 8, 12), 0.005),
        beard,
      );
    ...
    // Глаза мудрые
    for (const s of [-1, 1]) {
      const e = new Mesh(new SphereGeometry(0.028, 14, 10), eye);
    ...
    // Посох с кристаллом
    const staffMesh = new Mesh(trunkGeo(0.045, 0.06, 1.9), staff);
    staffMesh.position.set(0.5, 0.82, 0.08);
    staffMesh.rotation.z = 0.1;
    staffMesh.castShadow = true;
    this.body.add(staffMesh);

    const crystal = new Mesh(soft(new OctahedronGeometry(0.095, 2), 0.01), gem);
    crystal.position.set(0.57, 1.75, 0.12);
    this.body.add(crystal);

    // Обёртка посоха
    const wrap = new Mesh(soft(new CylinderGeometry(0.06, 0.06, 0.22, 14, 3), 0.01), mat('#c9a86a', 0.7));
  ...
  private buildMilica() {
    ...
    // Платье — капсюла
    const dressMesh = new Mesh(softCapsule(0.38, 0.55, 28), dress);
    dressMesh.position.y = 0.65;
    dressMesh.castShadow = true;
    this.body.add(dressMesh);

    // Лиф
    const top = new Mesh(softSphere(0.3, 26), dress);
    top.scale.set(1.0, 0.85, 0.95);
    top.position.y = 1.28;
    this.body.add(top);

    // Фартук
    const apronMesh = new Mesh(soft(new CylinderGeometry(0.32, 0.4, 0.6, 22), 0.02), apron);
    ...
    const mkArm = (side: number) => {
      ...
      const shoulder = new Mesh(softSphere(0.12, 18), dress);
      g.add(shoulder);
      const upper = new Mesh(softCapsule(0.08, 0.26, 16), dress);
      upper.position.y = -0.18;
      g.add(upper);
      const forearm = new Mesh(softCapsule(0.07, 0.24, 14), skin);
      forearm.position.y = -0.5;
      g.add(forearm);
      const hand = new Mesh(softSphere(0.06, 14), skin);
    ...
    // Голова
    const faceGeo = soft(new SphereGeometry(0.19, 28, 24), 0.015);
    const face = new Mesh(faceGeo, skin);
    face.castShadow = true;
    this.head.add(face);
    ...
    const hairMesh = new Mesh(
      soft(new SphereGeometry(0.21, 26, 20, 0, TAU, 0, Math.PI * 0.55), 0.02),
      hair,
    );
    ...
    // Коса
    const braid = new Mesh(softCapsule(0.045, 0.5, 12), hair);
    braid.position.set(-0.08, -0.38, -0.08);
    braid.rotation.x = 0.35;
    this.head.add(braid);

    // Лента
    const ribbonMesh = new Mesh(new BoxGeometry(0.1, 0.04, 0.08), ribbon);
    ribbonMesh.position.set(-0.08, -0.12, -0.05);
    this.head.add(ribbonMesh);

    // Глаза
    for (const s of [-1, 1]) {
      const eyeWhite = new Mesh(new SphereGeometry(0.028, 12, 10), mat('#eef2f6', 0.4));
      eyeWhite.position.set(s * 0.065, 0.03, 0.17);
      eyeWhite.scale.set(1.15, 0.7, 0.55);
      this.head.add(eyeWhite);

      const iris = new Mesh(new SphereGeometry(0.016, 12, 8), mat('#6aaa8a', 0.3));
    ...
    // Рот — мягкая улыбка
    const mouth = new Mesh(soft(new CapsuleGeometry(0.008, 0.035, 6, 10), 0.004), mat('#c48a6e', 0.6));
  ...
  private buildMelnik() {
    ...
    // Тело — шире
    const bodyMesh = new Mesh(softCapsule(0.44, 0.45, 26), tunic);
    bodyMesh.position.y = 0.7;
    bodyMesh.castShadow = true;
    this.body.add(bodyMesh);

    // Фартук
    const apronMesh = new Mesh(soft(new CylinderGeometry(0.36, 0.42, 0.7, 20), 0.02), apron);
    apronMesh.position.set(0, 0.75, 0.12);
    this.body.add(apronMesh);

    // Плечи
    const shoulders = new Mesh(softSphere(0.38, 26), tunic);
    ...
    const mkArm = (side: number) => {
      ...
      const shoulder = new Mesh(softSphere(0.13, 18), tunic);
      g.add(shoulder);
      const upper = new Mesh(softCapsule(0.1, 0.3, 16), tunic);
      upper.position.y = -0.2;
      g.add(upper);
      const forearm = new Mesh(softCapsule(0.09, 0.28, 14), skin);
      forearm.position.y = -0.52;
      g.add(forearm);
      const hand = new Mesh(softSphere(0.07, 14), skin);
    ...
    // Голова
    const face = new Mesh(softSphere(0.2, 28), skin);
    face.castShadow = true;
    this.head.add(face);

    // Шапка мельника — деформированная сфера
    const hatMesh = new Mesh(
      soft(new SphereGeometry(0.21, 24, 18, 0, TAU, 0, Math.PI * 0.55), 0.03),
      hat,
    );
    ...
    for (let i = 0; i < 3; i++) {
      const dot = new Mesh(
        new CircleGeometry(0.02, 8),
        new MeshBasicMaterial({ color: '#f5eeda', transparent: true, opacity: 0.6, depthWrite: false }),
      );
    ...
    // Глаза
    for (const s of [-1, 1]) {
      const e = new Mesh(new SphereGeometry(0.025, 12, 10), mat('#888', 0.4));
  ...
  private buildVillager(color: string) {
    ...
    const bodyMesh = new Mesh(softCapsule(0.36, 0.55, 24), tunic);
    bodyMesh.position.y = 0.72;
    bodyMesh.castShadow = true;
    this.body.add(bodyMesh);

    const shoulders = new Mesh(softSphere(0.28, 24), tunic);
    ...
    const mkArm = (side: number) => {
      ...
      const shoulder = new Mesh(softSphere(0.1, 16), tunic);
      g.add(shoulder);
      const arm = new Mesh(softCapsule(0.075, 0.52, 14), tunic);
      arm.position.y = -0.3;
      g.add(arm);
      const hand = new Mesh(softSphere(0.06, 12), skin);
    ...
    const face = new Mesh(softSphere(0.18, 24), skin);
    face.castShadow = true;
    this.head.add(face);

    const hairMesh = new Mesh(
      soft(new SphereGeometry(0.19, 22, 16, 0, TAU, 0, Math.PI * 0.5), 0.015),
      hair,
    );
    hairMesh.position.y = 0.03;
    this.head.add(hairMesh);
    ...
    for (const s of [-1, 1]) {
      const e = new Mesh(new SphereGeometry(0.022, 10, 8), mat('#888', 0.4));
  ...
  private buildKot() {
    ...
    const bodyMesh = new Mesh(softCapsule(0.2, 0.35, 18), fur);
    ...
    // Грудная клетка
    const chest = new Mesh(softSphere(0.18, 20), fur);
    chest.scale.set(1.15, 0.85, 0.8);
    chest.position.set(0, 0.3, 0.18);
    this.body.add(chest);

    // Голова
    const headMesh = new Mesh(softSphere(0.15, 22), fur);
    ...
    // Уши
    for (const s of [-1, 1]) {
      const ear = new Mesh(soft(new ConeGeometry(0.055, 0.14, 14), 0.01), furDark);
      ear.position.set(0.09 * s, 0.15, 0.3);
      ear.rotation.z = -0.3 * s;
      this.head.add(ear);
      // внутренность уха
      const innerEar = new Mesh(new SphereGeometry(0.03, 10, 8), mat('#e8a0a0', 0.7));
    ...
    for (const s of [-1, 1]) {
      const eye = new Mesh(new SphereGeometry(0.028, 14, 10), eyeGlow);
    ...
    // Нос
    const nose = new Mesh(softSphere(0.022, 10), mat('#c97a2e', 0.6));
    ...
    for (const s of [-1, 1]) {
      for (let w = 0; w < 3; w++) {
        const whisker = new Mesh(
          soft(new CapsuleGeometry(0.005, 0.12, 4, 6), 0.003),
          mat('#d9d2c4', 0.6),
        );
    ...
    // Хвост
    this.tail = new Group();
    const tailMesh = new Mesh(softCapsule(0.035, 0.45, 12), furDark);
    tailMesh.position.y = 0.22;
    tailMesh.castShadow = true;
    this.tail.add(tailMesh);
    // кончик хвоста
    const tailTip = new Mesh(softSphere(0.05, 12), fur);
  ...
  private buildKoza() {
    ...
    // Тело
    const bodyMesh = new Mesh(softCapsule(0.22, 0.5, 16), fur);
    bodyMesh.rotation.z = Math.PI / 2;
    ...
    for (const s of [-1, 1]) {
      for (const f of [-1, 1]) {
        const leg = new Mesh(softCapsule(0.04, 0.28, 10), furDark);
        leg.position.set(0.14 * s, 0.21, 0.25 * f);
        this.body.add(leg);
        // копыто
        const hoof = new Mesh(
          soft(new BoxGeometry(0.08, 0.04, 0.1, 4, 2, 4), 0.005),
          mat('#3e3e3e', 0.7),
        );
    ...
    // Голова
    const headMesh = new Mesh(softSphere(0.13, 22), fur);
    ...
    // Рога
    for (const s of [-1, 1]) {
      const hornMesh = new Mesh(soft(new ConeGeometry(0.03, 0.22, 14), 0.01), horn);
    ...
    // Уши
    for (const s of [-1, 1]) {
      const ear = new Mesh(soft(new ConeGeometry(0.04, 0.12, 14), 0.01), fur);
    ...
    for (const s of [-1, 1]) {
      const eyeWhite = new Mesh(new SphereGeometry(0.025, 12, 8), mat('#eef2f6', 0.4));
      eyeWhite.position.set(s * 0.06, 0.06, 0.1);
      eyeWhite.scale.set(1.2, 0.8, 0.6);
      this.head.add(eyeWhite);

      const pupil = new Mesh(
        new BoxGeometry(0.025, 0.008, 0.01),
        mat('#1a1a1a', 0.3),
      );
  ...
  setFollow(v: Vector3 | null) { this.follow = v; this.wait = 0; }

  update(dt: number, playerPos: Vector3, t: number) {
    if (this.follow) {
      const d = this.group.position.distanceTo(this.follow);
      if (d > 2.4) {
        const dir = new Vector3().subVectors(this.follow, this.group.position).normalize();
      ...
      if (this.wait > 0) {
        ...
        if (d < 0.4) {
          ...
          const dir = new Vector3(this.target.x - this.group.position.x, 0, this.target.z - this.group.position.z).normalize();
