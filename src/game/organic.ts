 ...
 */
import { BufferAttribute, BufferGeometry, CylinderGeometry, DoubleSide, Group, IcosahedronGeometry, Mesh, MeshStandardMaterial, SphereGeometry } from 'three';
...
export function rockGeo(size = 1, detail = 1) {
  const g = new IcosahedronGeometry(size, detail);
...
export function crownGeo(radius: number, seg = 14) {
  const g = new SphereGeometry(radius, seg, Math.floor(seg * 0.8));
...
export function trunkGeo(radiusTop: number, radiusBottom: number, height: number) {
  const g = new CylinderGeometry(radiusTop, radiusBottom, height, 8, 6, false);
...
export function roundedRoofGeo(width: number, height: number, depth: number) {
  ...
  const cap = new SphereGeometry(width * 0.72, 14, 10, 0, Math.PI * 2, 0, Math.PI * 0.5);
  cap.scale(1, height / (width * 0.72), depth / width);
  // основание — цилиндр снизу
  const base = new CylinderGeometry(width * 0.72, width * 0.72, height * 0.12, 14);
...
export function flowerGroup(petalColor: string, size = 0.3): Group {
  const g = new Group();
  const stemGeo = new CylinderGeometry(0.02, 0.025, size * 1.2, 5);
  deformGeometry(stemGeo, 0.01, 2, rand(0, 10));
  const stem = new Mesh(stemGeo, new MeshStandardMaterial({ color: '#4b6b3a', roughness: 1 }));
  stem.position.y = size * 0.6;
  g.add(stem);
  const head = new Group();
  const petalMat = new MeshStandardMaterial({ color: petalColor, roughness: 0.85 });
  for (let i = 0; i < 5; i++) {
    const a = (i / 5) * Math.PI * 2;
    const petal = new Mesh(new SphereGeometry(size * 0.22, 6, 5), petalMat);
    petal.scale.set(1, 0.35, 1.3);
    petal.position.set(Math.cos(a) * size * 0.22, 0, Math.sin(a) * size * 0.22);
    petal.rotation.y = -a;
    head.add(petal);
  }
  const core = new Mesh(new SphereGeometry(size * 0.14, 6, 5), new MeshStandardMaterial({ color: '#f2c14e', roughness: 0.7 }));
  head.add(core);
...
export function windGrassMaterial(color: string): MeshStandardMaterial {
...
export function updateWindMaterial(m: MeshStandardMaterial, t: number, wind: number) {
