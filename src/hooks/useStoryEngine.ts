 ...
 * Теперь квесты влияют на доступные выборы
 */
export function useStoryEngine({
  ...
  const checkChoiceCondition = useCallback((choice: StoryChoice): { available: boolean; lockReason?: string } => {
    ...
    if (condition.questActive && !activeQuestIds.includes(condition.questActive)) {
      return {
        available: false,
        lockReason: 'Требуется активный квест',
      };
    }
    
    // Квест должен быть завершён
    if (condition.questCompleted && !completedQuestIds.includes(condition.questCompleted)) {
      return {
        available: false,
        lockReason: 'Требуется завершённый квест',
      };
    }
    
    // Квест ещё не начат
    if (condition.questNotStarted) {
      if (activeQuestIds.includes(condition.questNotStarted) || 
          completedQuestIds.includes(condition.questNotStarted)) {
        return {
          available: false,
          lockReason: 'Квест уже начат',
        };
      }
    }
    
    // Квест не завершён (для блокировки пути)
    if (condition.questNotCompleted && completedQuestIds.includes(condition.questNotCompleted)) {
      return {
        available: false,
        lockReason: 'Другой путь уже выбран',
      };
    }
    
    // Минимум завершённых квестов
    if (condition.minQuestCount && completedQuestIds.length < condition.minQuestCount) {
      return {
        available: false,
        lockReason: `Нужно завершить ещё квестов (${completedQuestIds.length}/${condition.minQuestCount})`,
      };
    }
    
    // Проверка посещённого узла
    if (condition.visitedNode) {
      // TODO: добавить visitedNodes в параметры
      return { available: true };
    }
    
    return { available: true };
  }, [playerFlags, inventory, playerSkills, npcRelations, activeQuestIds, completedQuestIds]);
