  ...
  useEffect(() => {
    ...
    unsubs.push(
      eventBus.on('object:interact', ({ triggerZoneId }) => {
        ...
        // OR immediately when there is no examine+linked flow (user cannot get "stuck"
        // with the object burned before linked content runs — e.g. opening inventory
        // while ExaminePanel is open used to close the UI but leave the trigger consumed).
