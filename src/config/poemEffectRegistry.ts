...
export interface PoemTTLConsumerMeta {
  ...
  /**
   * [roadmap:STLTH-03] Multiplier applied to patrolling creep vision cone
   * range while this TTL flag is live (e.g. guiding_star_active → 0.45).
   * 1.0 = no effect. Read by PatrollingCreeps.tsx via getPoemTTLConsumer.
   * Makes stealth systemically responsive to ALL poem effects, not just
   * guiding_star.
   */
  creepVisionScale?: number;
...
export const POEM_TTL_CONSUMERS: Record<string, PoemTTLConsumerMeta> = {
  ...
  guiding_star_active: {
    ...
    /** [roadmap:STLTH-03] Shrink creep vision cone to 45% while active. */
    creepVisionScale: 0.45,
  },
  storm_wind_active: {
