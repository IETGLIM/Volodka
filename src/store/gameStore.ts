...
function buildGameSnapshot(state: GameStoreState): GameStoreSnapshot {
  return {
    ...
    playerState: { flags: state.playerState.flags, inventory: state.playerState.inventory, skills: state.playerState.skills, energy: state.playerState.energy, karma: state.playerState.karma, stress: state.playerState.stress, visitedNodes: state.playerState.visitedNodes, equippedThoughtIds: state.equippedThoughtIds ?? [], thoughtInternalizationPoints: state.thoughtInternalizationPoints ?? {}, progression: { level: state.playerState.progression?.level ?? 1, currentAct: state.playerState.progression?.currentAct ?? 1, skillPoints: state.playerState.progression?.skillPoints ?? 0, unlockedSkills: state.playerState.progression?.unlockedSkills ?? [], unlockedPerks: state.playerState.progression?.unlockedPerks ?? [], perkPoints: state.playerState.progression?.perkPoints ?? 0 }, choiceLog: state.playerState.choiceLog ?? [], moralChoices: state.playerState.moralChoices ?? [] },
