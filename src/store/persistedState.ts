...
export function storePatchFromSave(payload: SavePayload): Partial<GameStoreState> {
  ...
  // [roadmap:CMBT-07] Restore combat difficulty from save (not localStorage-only).
  // Also sync to localStorage so combatDifficulty.ts reads the correct value.
  if (payload.combatDifficulty) {
    patch.combatDifficulty = payload.combatDifficulty;
    try {
      localStorage.setItem('volodka_combat_difficulty', payload.combatDifficulty);
    } catch {
      /* ignore quota errors */
    }
  }
