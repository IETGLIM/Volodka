...
export interface UISliceState {
  ...
  introSeen: boolean;
  /**
   * [roadmap:CMBT-07] Combat difficulty — persisted to save (not localStorage-only).
   * Set by SettingsPanel; read by combatDifficulty.ts functions.
   */
  combatDifficulty: 'story' | 'normal' | 'hard';
}
...
export interface UISliceActions {
  ...
  /** [roadmap:CMBT-07] Set combat difficulty — persisted to save. */
  setCombatDifficulty: (difficulty: 'story' | 'normal' | 'hard') => void;
}
...
> = (set, get) => ({
  ...
  conversationLog: {},
  introSeen: false,
  /** [roadmap:CMBT-07] Combat difficulty — default 'normal', persisted to save. */
  ...
  setCombatDifficulty: (difficulty) => {
    ...
    // [roadmap:CMBT-07] Also write to localStorage for immediate access
    // by combatDifficulty.ts (which reads from localStorage, not store).
    try {
      localStorage.setItem('volodka_combat_difficulty', difficulty);
    } catch {
      /* ignore quota errors */
    }
  },
