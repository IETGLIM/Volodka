...
function createSkinMaterial(scatter: number): ShaderMaterial {
  ...
  return new ShaderMaterial({
    ...
    uniforms: {
      ...
      uSkinColor: { value: new Color('#c99574') },

      uScatter: { value: scatter },

      uLightDir: { value: new Vector3(0.4, 0.9, 0.2).normalize() },
...
function createClothMaterial(color: string, seed: number): ShaderMaterial {
  ...
  return new ShaderMaterial({
    ...
    uniforms: {
      ...
      uClothColor: { value: new Color(color) },
...
function buildHarmonicFace(radius: number, detail = 32): BufferGeometry {

  const geo = new SphereGeometry(radius, detail, detail);
...
function buildHairCap(radius: number): BufferGeometry {

  const geo = new SphereGeometry(radius, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.55);
...
export interface ProceduralCharacterProps {
  ...
  groundMeshesRef?: React.MutableRefObject<Object3D[]>;
...
export function ProceduralCharacter({
  ...
  const skinMats = useRef<ShaderMaterial[]>([]);

  const clothMats = useRef<ShaderMaterial[]>([]);

  const walk = useRef<WalkCycleState>(createWalkState(new Vector3(...position)));
  ...
  const bodyGeo = useMemo(() => new CapsuleGeometry(0.2, 0.42, 8, 14), []);

  const shirtGeo = useMemo(() => new CapsuleGeometry(0.225, 0.38, 6, 12), []);

  const pantsGeo = useMemo(() => new CapsuleGeometry(0.075, 0.32, 4, 8), []);

  const limbGeo = useMemo(() => new CapsuleGeometry(0.055, 0.32, 4, 8), []);
  ...
  useEffect(() => {
    ...
    const knee = new Vector3(-0.12, 0.48, 0);

    const foot = new Vector3(-0.12, 0.02, 0.02);
