...
// ═══════════════════════════════════════════════════
//  RAIN STREAKS — fast-falling elongated particles
// ═══════════════════════════════════════════════════

interface RainConfig {
  count: number;
  area: [number, number]; // x/z spread
  height: number;
  fallSpeed: number;
  windSpeed: number;
  color: string;
...
function RainSystem({ config }: { config: RainConfig }) {
  const pointsRef = useRef<THREE.Points>(null);
  const materialRef = useRef<THREE.PointsMaterial>(null);
  const timeRef = useRef(0);
  ...
  const positions = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      const i3 = i * 3;
      pos[i3] = (Math.random() - 0.5) * config.area[0];
      pos[i3 + 1] = Math.random() * config.height;
      pos[i3 + 2] = (Math.random() - 0.5) * config.area[1];
    }
    return pos;
  }, [config]);
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Wind drift
      posArray[i3] += config.windSpeed * delta;

      // Reset at ground
      if (posArray[i3 + 1] < 0) {
        posArray[i3] = (Math.random() - 0.5) * config.area[0];
        posArray[i3 + 1] = config.height;
        posArray[i3 + 2] = (Math.random() - 0.5) * config.area[1];
        if (velocitiesRef.current) velocitiesRef.current[i] = config.fallSpeed * (0.8 + Math.random() * 0.4);
      }
    }
    ...
    if (materialRef.current) {
      materialRef.current.opacity = 0.15 + Math.sin(timeRef.current * 0.3) * 0.03;
    }
  });
...
// ═══════════════════════════════════════════════════
//  SNOW DRIFT — gently falling snow particles
// ═══════════════════════════════════════════════════
...
const SNOW_CONFIGS: Record<string, SnowConfig> = {
  street_winter: {
    count: 200,
    area: [30, 30],
    height: 12,
...
function SnowSystem({ config }: { config: SnowConfig }) {
  const pointsRef = useRef<THREE.Points>(null);
  const materialRef = useRef<THREE.PointsMaterial>(null);
  const timeRef = useRef(0);
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      posArray[i3] += (config.windSpeed + Math.sin(t * 0.5 + phase) * 0.3) * delta;
      posArray[i3 + 2] += Math.cos(t * 0.4 + phase * 1.2) * 0.2 * delta;
      ...
      if (posArray[i3 + 1] < 0) {
        ...
        posArray[i3 + 1] = config.height;
        posArray[i3 + 2] = (Math.random() - 0.5) * config.area[1];
        if (velocitiesRef.current) velocitiesRef.current[i] = config.fallSpeed * (0.5 + Math.random() * 1.0);
      }
    }
    ...
    if (materialRef.current) {
      materialRef.current.opacity = 0.35 + Math.sin(t * 0.2) * 0.05;
    }
  });
